...
export const createImage: RequestHandler = async (req, res) => {
  try {
    ...
    const uploadResult = await cloudinary.uploader.upload(file.path, {
      folder: "event_images", // Carpeta en Cloudinary
    });
    ...
    const newImage = new ImagesEvents();
...
export const getImages: RequestHandler = async (req, res) => {
  try {
    const images = await ImagesEvents.find({ relations: ["event"] });
    res.json(images);
  } catch (err) {
    if (err instanceof Error) {
      res.status(500).json({ message: err.message });
...
export const getImagesFullData: RequestHandler = async (req, res) => {
  try {
    const images = await ImagesEvents.createQueryBuilder("image")
...
export const updateImage: RequestHandler = async (req, res) => {
  ...
  try {
    const image = await ImagesEvents.findOneBy({ id: parseInt(id) });
    ...
    await ImagesEvents.update({ id: parseInt(id) }, req.body);
